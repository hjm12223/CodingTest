...
public class NewsClustering {
	...
	public static int solution(String str1, String str2) {
		Map<String, Integer> comb1 = new HashMap<>();
		Map<String, Integer> comb2 = new HashMap<>();
		Map<String, Integer> staticComb1;

		Map<String, Integer> hap = new HashMap<>();
		String[] firstSplit = str1.split("");
		String[] secondSplit = str2.split("");

		pair(firstSplit, comb1, hap);
		staticComb1 = Map.copyOf(comb1);

		pair(secondSplit, comb2, hap);

		int sum = calSum(staticComb1, comb2); // 교집합
		int union = calUnion(comb1, comb2, sum); // 합집합

		if (union == 0) return 65536;
		float jacard = (float) sum / union;
		return Math.round(jacard * 65536* 1000 ) / 1000;
	}

	private static void pair(String[] split, Map<String, Integer> comb, Map<String, Integer> hap) {
		for (int i = 0; i < split.length - 1; i++) {
			String pair = split[i].toUpperCase() + split[i + 1].toUpperCase();
			if (pair.matches("[A-Z]{2}")) {
				comb.put(pair, comb.getOrDefault(pair, 0) + 1);
				hap.put(pair, hap.getOrDefault(pair, 0) + 1);
			}
		}
	}


	private static int calSum(Map<String, Integer> comb1, Map<String, Integer> comb2) {
		int sum = 0;
		for (String key : comb1.keySet()) {
			if (comb2.containsKey(key)) {
				sum += Math.min(comb1.get(key), comb2.get(key));
			}
		}
		return sum;
	}

	private static int calUnion(Map<String, Integer> comb1, Map<String, Integer> comb2, int sum) {
		int union = 0;
		for (int value : comb1.values()) {
			union += value;
		}
		for (int value : comb2.values()) {
			union += value;
		}
		return union - sum;
	}
