...
public class StringZip {
    public static void main(String[] args) {
        int solution = solution("aabbaccc");
    ...
    public static int solution(String s) {
        ...
        String temp = s;
         /*
          분기점을 어떻게 나눌것이냐
          1개씩 2개씩 3개씩 4개씩 .....
          이렇게 잘랐을 경우 어떤 경우가 가장 많이 압축되는지를 확인
          최대 N/2 의 경우의 문자열이 나오니 N/2까지 반복문을 돌림
           */
        for (int i = 1; i <= n / 2; i++) {
            ...
            for (int j = i; j < n; j += i) { // j 는 i부터 시작하고 +i의 문자열의 경우의 수를 찾음

                String sub = s.substring(j, Math.min(j + i, n)); // j+i가 길이를 초과할 수도 있으니

                if (preStr.equals(sub)) { // 같은 문자열이면 count++;
                    count++;
                }
                else { // 아닐경우
                    if (count >1){
            ...
            } // 해당 문자열의 경우의 수를 다 찾았을 경우
            if (count >1){ // 만약 카운트가 1 초과일 경우
                result.append(count);
                result.append(preStr);
            }else { // 아닐경우
                result.append(preStr);
