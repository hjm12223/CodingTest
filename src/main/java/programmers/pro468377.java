...
public class pro468377 {
	static int N;
	static int result = Integer.MAX_VALUE;
	static int[] coupon;

	/*
1. 스테이지가 존재하며 각 스테이지에서는 다음 스테이지에 관한 힌트권을 구매할 수 있음
2. 힌트권을 사용하여 해당 스테이지 해결 비용을 절감 할 수 있음.
...
if(!map.contains(i+1))
    ...
    result += cost[i][map.get(i+1)];
}
map.putIfAbsent(hint[i][j],map.getOrDefault(hint[i][j],0)+1);
*/
	...
	static void dfs(int[][] cost, int[][] hint, int depth, int value) {
		if (depth == N) {
		...
		int use = Math.min(coupon[i + 1], cost[i].length - 1);

		dfs(cost, hint, depth + 1, value + cost[i][use]);
		...
		if (i < hint.length) {
			...
			dfs(cost, hint, depth + 1, value + cost[i][use] + hint[i][0]);
